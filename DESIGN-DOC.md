...
### Assumption:
**Single Chessboard**: - The implementation uses static methods in the `ChessBoard` and `ChessPiece` classes. - This design assumes that only one chessboard is in use at any given time. - Managing multiple chessboards simultaneously would require instance-based methods or a different design pattern.
